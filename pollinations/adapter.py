...
from repositories.ipfs import (
    get_local_folder_for_cid,
    download_cid,
    delete_downloaded_cid,
    DownloadedCID,
)
...
def extract_cid_meta(cid):
    ...
    files_and_folders_df = files_and_folders_df[
        ~files_and_folders_df.str.contains("\.")
    ]
...
def process_cid(task: Task):
    with DownloadedCID(task.cid) as d:
        if is_cid_eligible(task.cid):
            ...
            content_in_db = Content.objects.get_or_create(
                cid=task.cid, defaults=cid_metadata
            )
